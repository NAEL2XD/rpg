...
class Entering extends FlxSprite {
    ...
    public function new(data:EnteringData) {
        ...
        snd = FlxG.sound.load('assets/sounds/${data.soundIfEnter == null ? "door_close" : data.soundIfEnter}.ogg');
    ...
    public function checkMovement() {
        ...
        if (FlxMath.distanceBetween(dataStr.player, this) < 80 && FlxG.keys.anyJustPressed([Z, W, UP]) && !dataStr.player.cutscene) {
            ...
            FlxG.sound.play('assets/sounds/${dataStr.soundIfEnter == null ? "door_open" : dataStr.soundIfEnter}.ogg');
